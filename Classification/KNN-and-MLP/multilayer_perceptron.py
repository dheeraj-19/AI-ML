...
class MultilayerPerceptron:
    ...
    def __init__(self, n_hidden = 16, hidden_activation = 'sigmoid', n_iterations = 1000, learning_rate = 0.01):
        ...
        self.hidden_activation = relu #activation_functions[hidden_activation]
    ...
    def _initialize(self, X, y):
        ...
        t = 1 / math.sqrt(n_features)
        
        self._h_weights = np.random.uniform(-t, t, (n_features, self.n_hidden))
        self._h_bias = np.ones((1, self.n_hidden))
        
        t = limit   = 1 / math.sqrt(self.n_hidden)
        
        self._o_weights = np.random.uniform(-t, t, (self.n_hidden, n_outputs))
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_iterations):
            ...
            h_ip = self._X.dot(self._h_weights) + self._h_bias
            h_op = self.hidden_activation(h_ip)
            
            o_ip = h_op.dot(self._o_weights) + self._o_bias
            y_pred = self._output_activation(o_ip)
            ...
            gradient_o_ip = loss * self._output_activation(o_ip, True)
            gradient_o_weights = h_op.T.dot(gradient_o_ip)
            gradient_o_bias = np.sum(gradient_o_weights, axis=0, keepdims=True)
            
            gradient_h_ip = gradient_o_ip.dot(self._o_weights.T) * self.hidden_activation(h_ip, True)
            gradient_h_weights = self._X.T.dot(gradient_h_ip)
            gradient_h_bias = np.sum(gradient_h_ip, axis=0, keepdims=True)
            
            self._o_weights -= self.learning_rate * gradient_o_weights
            self._o_bias -= self.learning_rate * gradient_o_bias
            self._h_weights -= self.learning_rate * gradient_h_weights
            self._h_bias -= self.learning_rate * gradient_h_bias
    ...
    def predict(self, X):
        ...
        h_ip = X.dot(self._h_weights) + self._h_bias
        h_op = self.hidden_activation(h_ip)
